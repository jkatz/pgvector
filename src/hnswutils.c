	...
	while (!pairingheap_is_empty(C))
	{
		...
		for (int i = 0; i < unvisitedLength; i++)
		{
			...
			else
			{
				...
				eElement = NULL;
				HnswLoadElementImpl(blkno, offno, &eDistance, q, index, support, inserting, alwaysAdd || discarded != NULL || nkeys > 0 ? NULL : &f->distance, &eElement);
			...
			/*
			 * ACORN-1: filter candidates by predicate. Only matching
			 * candidates proceed to the C/W insertion logic below.
			 */
			if (nkeys > 0 && !inserting && !inMemory)
			{
				bool		matches = false;

				for (int j = 0; j < eElement->heaptidsLength; j++)
				{
					if (HnswEvaluatePredicates(&eElement->heaptids[j],
											   heapRel, snapshot, keys, nkeys))
					{
						matches = true;
						break;
					}
