...
GetScanItems(IndexScanDesc scan, Datum value)
{
	...
	return HnswSearchLayer(base, q, ep, hnsw_ef_search, 0, index, support, m, false, NULL, &so->v, hnsw_iterative_scan != HNSW_ITERATIVE_SCAN_OFF ? &so->discarded : NULL, true, &so->tuples, so->heapRelation, scan->xs_snapshot, so->keys, so->nkeys);
}
